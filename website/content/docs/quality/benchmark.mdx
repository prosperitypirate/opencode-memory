...
> These environment variables are read by the benchmark runner directly (`benchmark/src/`), not by the codexfi plugin. The plugin reads keys from `~/.config/opencode/codexfi.jsonc`. You need both: the config file for the plugin, and the env vars for the benchmark script.
